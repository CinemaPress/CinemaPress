  ...
  return function(req, res, next) {
    ...
    if (
      ...
      if (!(cache.itemCount % 100)) {
        ...
        if (cache.itemCount) {
          console.log('DUMP WHOIS WRITE', cache.itemCount);
          fs.writeFileSync(whois_dump, JSON.stringify(cache.dump()));
        } else if (fs.existsSync(whois_dump)) {
          var whois_raw = fs.readFileSync(whois_dump, 'utf8');
          try {
            cache.load(JSON.parse(whois_raw));
            console.log('DUMP WHOIS READ', cache.itemCount);
          } catch (e) {
            console.error(e);
          }
        }
      ...
      whois.lookup(req.userinfo.ip, function(err, data) {
        if (err) {
          req.userinfo.whois = '';
          console.error('NOT WHOIS', req.userinfo.ip);
        } else {
