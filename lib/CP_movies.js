              ...
              axios((task.id + '').replace(/\[[a-z0-9_]+?]/i, id))
                .then(function(j) {
                  ...
                  if (movie['kp_id'] && parseInt(movie['kp_id'])) {
                    req['id'] = parseInt(movie['kp_id']) + '';
                  } else if (
                    movie['custom'] &&
                    movie['custom']['tmdb_id'] &&
                    parseInt(movie['custom']['tmdb_id'])
                  ) {
                    req['id'] = 'custom.tmdb_id';
                    req[req['id']] = parseInt(movie['custom']['tmdb_id']) + '';
                  } else if (
                    movie['custom'] &&
                    movie['custom']['imdb_id'] &&
                    parseInt(movie['custom']['imdb_id'])
                  ) {
                    req['id'] = 'custom.imdb_id';
                    req[req['id']] = parseInt(movie['custom']['imdb_id']) + '';
                  } else if (
                    movie['custom'] &&
                    movie['custom']['douban_id'] &&
                    parseInt(movie['custom']['douban_id'])
                  ) {
                    req['id'] = 'custom.douban_id';
                    req[req['id']] =
                      parseInt(movie['custom']['douban_id']) + '';
                  } else if (
                    movie['custom'] &&
                    ...
                    req['id'] = 'custom.wa_id';
                    req[req['id']] = parseInt(movie['custom']['wa_id']) + '';
                  } else if (
                    movie['custom'] &&
                    movie['custom']['movie_id'] &&
                    parseInt(movie['custom']['movie_id'])
                  ) {
                    req['id'] = 'custom.movie_id';
                    req[req['id']] = parseInt(movie['custom']['movie_id']) + '';
                  }
                  CP_get.movies(req, 1, '', 1, false, function(err, rt) {
                    if (err) {
                      console.error(err);
                      return callback();
                    }
