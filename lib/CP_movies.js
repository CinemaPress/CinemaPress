                  ...
                  if (joinItem) {
                    set_data = listItem
                      .map(function(item) {
                        ...
                        if (sup_parse_no[0] && sup_parse_no[1]) {
              ...
              async.eachOfLimit(
                ...
                function() {
                  if (current_movie) {
                    ...
                    if (
                      (config.publish.required.indexOf('title_ru') + 1 &&
                        update_movie['title_ru'] === '') ||
                      (config.publish.required.indexOf('description') + 1 &&
                        update_movie['description'] === '') ||
                      (config.publish.required.indexOf('player') + 1 &&
                        update_movie['player'] === '') ||
                      (config.publish.required.indexOf('poster') + 1 &&
                        (update_movie['poster'] === '' ||
                          update_movie['poster'] === '0'))
                    ) {
                      console.log(
                        '[REALTIME]',
                        id_index + 1,
                        ...
                        [
                          config.publish.required.indexOf('title_ru') + 1
                            ? 'title_ru=' + (update_movie['title_ru'] === '')
                            : '',
                          config.publish.required.indexOf('description') + 1
                            ? 'description=' +
                              (update_movie['description'] === '')
