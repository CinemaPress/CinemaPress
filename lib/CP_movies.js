...
(function getInfo() {
  ...
  config.movies.cron.forEach(function(task) {
    ...
    tasks.push({
      ...
      array_url: ((parse[3] || '').split('<>')[1] || '').trim(),
      one_url: ((parse[3] || '').split('<>')[2] || '').trim(),
      format_url: ((parse[3] || '').split('<>')[3] || '')
        .replace(/POST>/i, '')
        .trim(),
      method_page: /POST>/i.test(parse[1] || '') ? 'POST' : 'GET',
      method_id:
        /POST>/i.test(parse[3] || '') ||
        /POST>/i.test((parse[3] || '').split('<>')[2] || '')
          ? 'POST'
          : 'GET',
      ids: [],
            ...
            axios(url_req, opt)
                  ...
                  if (
                    ...
                    if (r_data && (!task.id || task.id === '[url]')) {
                      if (!task.path || task.id === '[url]') {
                        if (task.array_url) {
                          r_data = op.get(r_data, task.array_url);
                        }
                        if (
                          ...
                          if (task.one_url) {
                            ...
                            r_data = r_data
                              .map(function(r) {
                                var one_url = op.get(r, task.one_url);
                                if (!one_url) return;
                                var format_url = task.format_url
                                  ? task.format_url
                                      .replace(/_VALUE_/g, one_url)
                                      .replace(/(^"*)|("*)$/g, '')
                                  : one_url;
                                if (
                                  task.additional_info &&
                                  ...
                                  var ai = {};
                                  task.additional_info.forEach(function(key) {
                                    op.set(ai, key, op.get(r, key));
                                  });
                                  additional_info[format_url] = ai;
                                }
                                return format_url;
                              })
                              .filter(Boolean);
              ...
              .catch(function(err) {
                ...
                console.error(
                  ...
                  'STOP PAGE (' + (6 - fail_req) + ' fails)',
                  url_req,
                  (err.response && err.response.status) || err.code || err,
                  movies_proxy ? 'PROXY ' + movies_proxy : ''
                );
                fail_req = fail_req - 1;
                setTimeout(function() {
          ...
          function(id, id_index, callback) {
                      ...
                      if (Array.isArray(movies_on_page)) {
                        async.eachOfLimit(
                          ...
                          function(movie, id_index, callback) {
                            if (additional_info && additional_info[task_url]) {
                              movie = Object.assign({}, movie, {
                                additional_info: additional_info[task_url]
                              });
                            }
            ...
            function movie_data(json, callback) {
              ...
              if (movie['kp_id'] && parseInt(movie['kp_id'])) {
                ...
                if (
                  config.movies.id &&
                  config.movies.id === 'kp_id' &&
                  op.get(movie, config.movies.id)
                ) {
                  req_id = movie['kp_id'] && parseInt(movie['kp_id']);
                }
              }
              if (
                movie['custom'] &&
                ...
                if (
                  config.movies.id &&
                  config.movies.id === 'custom.tmdb_id' &&
                  op.get(movie, config.movies.id)
                ) {
                  req_id =
                    (movie['custom'] &&
              ...
              if (
                ...
                if (
                  config.movies.id &&
                  config.movies.id === 'custom.imdb_id' &&
                  op.get(movie, config.movies.id)
                ) {
                  req_id =
                    movie['custom'] &&
              ...
              if (
                ...
                if (
                  config.movies.id &&
                  config.movies.id === 'custom.douban_id' &&
                  op.get(movie, config.movies.id)
                ) {
                  req_id =
                    movie['custom'] &&
              ...
              if (
                ...
                if (
                  config.movies.id &&
                  config.movies.id === 'custom.tvmaze_id' &&
                  op.get(movie, config.movies.id)
                ) {
                  req_id =
                    movie['custom'] &&
                    movie['custom']['tvmaze_id'] &&
                    parseInt(movie['custom']['tvmaze_id']) &&
                    parseInt(movie['custom']['tvmaze_id']) + 700000000;
                }
              }
              if (
                movie['custom'] &&
              ...
              if (
                ...
                if (
                  config.movies.id &&
                  config.movies.id === 'custom.movie_id' &&
                  op.get(movie, config.movies.id)
                ) {
                  req_id =
                    movie['custom'] &&
              ...
              async.eachOfLimit(
                        ...
                        if (task.page === 'xmlpipe2') {
                          ...
                          connection.query(
                            ...
                            function(err2, rt2) {
                              ...
                              if (rt2 && rt2.length) {
                                current_movie = JSON.parse(
                                  JSON.stringify(rt2[0])
                                );
                                return callback('STOP');
                              }
                              if (rt && rt.length) {
                          ...
                          if (rt && rt.length) {
                            current_movie = JSON.parse(JSON.stringify(rt[0]));
                            return callback('STOP');
                          }
                          return callback();
                        }
                ...
                function() {
                  if (current_movie) {
                    ...
                    var cm = JSON.parse(JSON.stringify(current_movie));
                    delete cm['custom'];
                    ...
                    var parse_movie = JSON.parse(JSON.stringify(movie));
                    ...
                    if (current_movie.custom) {
                      if (
                        ...
                        current_movie_custom = JSON.parse(
                          JSON.stringify(current_movie.custom)
                        );
                      }
                    }
                    if (current_movie.custom && parse_movie.custom) {
                      ...
                      if (
                        ...
                        parse_movie_custom = JSON.parse(
                          JSON.stringify(parse_movie.custom)
                        );
                      }
                      var season = '';
                      var episode = '';
                      ...
                      Object.keys(parse_movie_custom).forEach(function(k1) {
                        if (
                          typeof parse_movie_custom[k1] === 'string' &&
                          parse_movie_custom[k1] === 'none'
                        ) {
                          parse_movie_custom[k1] = '';
                          delete current_movie_custom[k1];
                          return;
                        }
                        if (
                          typeof parse_movie_custom[k1] === 'string' &&
                          typeof current_movie_custom[k1] === 'string' &&
                          parse_movie_custom[k1] !== current_movie_custom[k1]
                        ) {
                          delete current_movie_custom[k1];
                          return;
                        }
                        if (
                          typeof parse_movie_custom[k1] === 'object' &&
                          !Array.isArray(parse_movie_custom[k1])
                        ) {
                          Object.keys(parse_movie_custom[k1]).forEach(function(
                            k2
                          ) {
                            if (
                              typeof parse_movie_custom[k1][k2] === 'string' &&
                              parse_movie_custom[k1][k2] === 'none'
                            ) {
                              if (
                                typeof current_movie_custom[k1] === 'object'
                              ) {
                                parse_movie_custom[k1][k2] = '';
                                delete current_movie_custom[k1][k2];
                                return;
                              }
                            }
                            if (
                              typeof parse_movie_custom[k1][k2] === 'string' &&
                              typeof current_movie_custom[k1] === 'object' &&
                              typeof current_movie_custom[k1][k2] ===
                                'string' &&
                              parse_movie_custom[k1][k2] !==
                                current_movie_custom[k1][k2]
                            ) {
                              delete current_movie_custom[k1][k2];
                            }
                          });
                      ...
                      current_movie.custom = deepmerge(
                        parse_movie_custom,
                        current_movie_custom
                      );
                      if (
                    ...
                    var cm2 = JSON.parse(JSON.stringify(update_movie));
                    delete cm2['custom'];
