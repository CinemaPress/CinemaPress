...
module.exports = function() {
  return function(req, res, next) {
    ...
    if (success_ip && success_ip.message) {
      if (success_ip.message === 1) {
        req.userinfo.bot.all = true;
        req.userinfo.bot.main = true;
      } else if (success_ip.message === 2) {
    ...
    if (bot_main.ips && bot_main.ips.length) {
      if (bot_main.ips.indexOf(req.userinfo.ip) === -1) {
        console.log(
