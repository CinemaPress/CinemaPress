...
function createImgUrl(type, size, id) {
  id = id ? ('' + id).trim() : '';
  var save = config.image.save ? '?save' : '';
  var image = '/files/poster/no-poster.jpg';
  var source = 'not';

  if (/^(http|\/\/)/i.test(id)) {
    source = 'url';
  } else if (
    config.image.addr === 'image.tmdb.org' &&
    /^\/[a-z0-9]*\.(jpg|png)$/i.test(id)
  ) {
    source = 'tmdb';
  } else if (
    config.image.addr === 'm.media-amazon.com' &&
    /^\/[a-z0-9@.,_\-]*\.(jpg|png)$/i.test(id)
  ) {
    source = 'imdb';
  } else if (/^(\/)/i.test(id)) {
    source = 'url';
  } else if (config.image.addr === 'st.kp.yandex.net') {
    source = 'kinopoisk';
  } else if (config.image.addr === config.domain) {
    source = 'server';
  }

  switch (source) {
    case 'kinopoisk':
      image = '/files/' + type + '/' + size + '/' + id + '.jpg' + save;
      break;
    case 'imdb':
    case 'tmdb':
      image = '/files/' + type + '/' + size + id + save;
      break;
    case 'server':
      if (size === 'original') {
        image = '/files/' + type + '/' + size + '/' + id + '.jpg' + save;
      } else {
        image = '/images/' + type + '/' + size + '/img' + id + '.jpg';
      }
      break;
    case 'url':
      image = id;
