  ...
  return movies.map(function(movie) {
    ...
    try {
      ...
      if (movie.custom['player1']) {
        movie.player += (movie.player ? ',' : '') + movie.custom['player1'];
      }
      if (movie.custom['player2']) {
        movie.player += (movie.player ? ',' : '') + movie.custom['player2'];
      }
      if (movie.custom['player3']) {
        movie.player += (movie.player ? ',' : '') + movie.custom['player3'];
      }
      if (movie.custom['player4']) {
        movie.player += (movie.player ? ',' : '') + movie.custom['player4'];
      }
      if (movie.custom['player5']) {
        movie.player += (movie.player ? ',' : '') + movie.custom['player5'];
      }
    } catch (e) {
      movie.custom = {};
    }
  ...
  function createCategoryUrl(type, items) {
    ...
    itemsArr = itemsArr.map(function(item) {
      return (
        ...
        (config.default.people_image &&
        (type === 'actor' || type === 'director')
          ? '<span style="position:absolute;visibility:hidden"><img src="/files/poster/people/' +
