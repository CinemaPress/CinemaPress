...
function structureMovie(movies, options) {
  ...
  function createTagUrl(movie) {
    ...
    var tags = [];
    var arr = [];
    var type = movie.type && movie.type.toString() === '1'
      ? config.urls.types.serial
      : config.urls.types.movie;

    [movie.year].forEach(function(year) { if (!year) return;
      if (config.default.tags.list.indexOf('year') + 1) {
        arr.push([{k: 'year', v: year}]);
      }
      movie.genre.split(',').forEach(function(genre, g) { if (!genre) return;
        if (config.default.tags.list.indexOf('genre') + 1) {
          arr.push([{k: 'genre', v: genre}]);
        }
        if (config.default.tags.list.indexOf('year') + 1 &&
          config.default.tags.list.indexOf('genre') + 1) {
          arr.push([{k: 'year', v: year}, {k: 'genre', v: genre}]);
        }
        movie.country.split(',').forEach(function(country) { if (!country) return;
          ...
          if (config.default.tags.list.indexOf('year') + 1 &&
            ...
            if (g === 0) arr.push([{k: 'year', v: year}, {k: 'country', v: country}]);
          }
          if (config.default.tags.list.indexOf('genre') + 1 &&
            config.default.tags.list.indexOf('country') + 1) {
            arr.push([{k: 'genre', v: genre}, {k: 'country', v: country}]);
          }
          if (config.default.tags.list.indexOf('year') + 1 &&
            config.default.tags.list.indexOf('genre') + 1 &&
            config.default.tags.list.indexOf('country') + 1) {
            arr.push([{k: 'year', v: year}, {k: 'genre', v: genre}, {k: 'country', v: country}]);
          }
        });
      });
    });
    ...
    arr.forEach(function(a) {
      ...
      tags.push('<a href="' +
        ...
        a.map(function(obj) {
          return obj.k + '=' + CP_translit.text(obj.v, undefined, config.urls[obj.k])
        }).join('&') +
        '">' +
  ...
  if (type === 'picture') {
    ...
    if (('' + id).replace(/\d/g, '')) {
      if (/^\/[a-z0-9]*\.(jpg|png)$/i.test('' + id)) {
        ...
        return config.protocol + config.subdomain + config.domain + id;
      }
    }
    if (st !== config.domain) {
      switch (size) {
