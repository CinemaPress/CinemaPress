              ...
              request(
                ...
                function(error2, response2, body2) {
                  ...
                  while ((found = regex_id.exec(body2)) !== null) {
                    ...
                    if (find_id && ids.indexOf(find_id) === -1) {
                      ids.push(find_id);
                    }
                  }
                  setTimeout(function() {
...
/**
 * Create random botdomain.
 */

if ((config.botdomain || config.subdomain) && hour === 5) {
  active.num++;
  ...
  request(
    ...
    function(error, response, body) {
      ...
      if (danger) {
        config.botdomain =
          domains.bot.second + domains.bot.main + domains.bot.domain;
        config.subdomain =
          domains.sub.second + domains.sub.main + domains.sub.domain;

        console.log('NEW SUBDOMAIN:', config.subdomain);
        console.log('NEW BOTDOMAIN:', config.botdomain);
