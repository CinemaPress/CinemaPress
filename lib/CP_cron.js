...
if (
  ...
  var keys = {
    ...
    iframe_movies: [
      'https://iframe.video/api/v2/updates?type=movie&limit=99&api_token=[token]'
    ],
    iframe_serials: [
      'https://iframe.video/api/v2/updates?type=serial&limit=99&api_token=[token]'
    ],
    videocdn_movies: [
...
if (modules.player.data.iframe.token && hour === 5) {
  ...
  request(
    { url: 'https://' + url + '/api/v2/translates', timeout: 15000 },
    function(error, response, translators) {
      if (error) {
        active.num--;
    ...
    if (headers_ && headers_.length) {
      ...
      async.eachOfLimit(
        ...
        function(d, i, callback) {
          var re = new RegExp(
            config.domain +
              '(\\/mobile-version|\\/tv-version|\\/app-version|)\\/[a-z0-9._-]*?' +
              (config.urls.slash === '/' ? '\\/' : '-') +
              '[a-z0-9._-]*',
            'ig'
          );
                ...
                function(unq, key, callback) {
                  ...
                  array_mvs.forEach(function(mvs) {
                    if (
                      ...
                      var custom = null;
                      try {
                        custom = unq.custom ? JSON.parse(unq.custom) : {};
                        custom.unique = custom.unique || false;
                      } catch (e) {
                        custom = null;
                      }
                      mov = {
