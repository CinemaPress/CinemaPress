...
if (
    ...
    imap.openBox('INBOX', true, function(err) {
      ...
      imap.search(search, function(err, results) {
        if (err || !results || !results.length) {
          console.log('imap.search', err || results);
          return imap.end();
        }
        ...
        f.on('message', function(msg) {
          ...
          parser.on('headers', headers => {
            headers_.push({
              to:
                ...
                headers.get('reply-to').value[0] &&
                headers.get('reply-to').value[0].address
                  ? headers.get('reply-to').value[0].address
                  : headers.get('from') &&
                    headers.get('from').value &&
                    headers.get('from').value[0] &&
          ...
          parser.on('data', function(data) {
            if (data.type === 'text') {
              data_.push(data.html || data.text || '');
            }
          });
  ...
  imap.once('end', function() {
    if (
      ...
      data_ &&
      data_.length &&
      headers_.length === data_.length
    ) {
      var save = false;
      async.eachOfLimit(
        headers_,
        1,
        function(h, i, callback) {
          var re = new RegExp(
            config.domain +
              '\\/.*?' +
              (config.urls.slash === '/' ? '\\/' : '-') +
          ...
          urls.forEach(function(u) {
            ...
            if (
              ...
              console.log('ABUSE', headers_[i].to, id);
