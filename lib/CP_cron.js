...
if (
  ...
  var keys = {
    ...
    hdvb_movies: ['http://farsihd.info/api/movies_updates.json?token=[token]'],
    hdvb_serials: ['http://farsihd.info/api/serials_updates.json?token=[token]']
  };
        ...
        async.eachOf(
          ...
          function(url, id, callback) {
            ...
            request({ url: url, method: 'GET', timeout: 5000 }, function(
              ...
              movies = tryParseJSON(movies);
              if (response.statusCode === 200 && movies) {
                movies.forEach(function(movie) {
                  if (
                    movie &&
                    movie.kinopoisk_id &&
