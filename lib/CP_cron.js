              ...
              if (p.regex_url) {
                ...
                while ((u = regex_url.exec(html)) !== null) {
                  if (/(\/\/|http)/i.test(u[1])) {
                    ...
                    urls.push(require('url').parse(p.url).origin + u[1]);
                  } else {
                    var s = p.url.split('/');
                    s.pop();
                    var slash = s.join('/');
...
if (
  ...
  var keys = {
    ...
    iframe_movies: [
      'https://iframe.video/api/v2/updates?type=serials&api_token=[token]'
    ],
    iframe_serials: [
    ...
    function(err) {
      ...
      if (all.length === 1) {
        ...
        all_movies = all_movies.slice(
          0,
          parseInt(modules.content.data.auto.iframe_movies.count)
        );
        saveContent('iframe_movies', all_movies, true, function() {
        ...
        if (ser.length === 1) {
          ...
          all_serials = all_serials.slice(
            0,
            parseInt(modules.content.data.auto.iframe_serials.count)
          );
          saveContent('iframe_serials', all_serials, true, function() {
...
if (modules.player.data.iframe.token && hour === 3) {
  active.num++;
  active.num++;
...
if (
  ...
  var date = new Date();
  var search = ['ALL', ['SINCE', date.setDate(date.getDate() - 1)]];

  if (modules.abuse.data.imap.from && modules.abuse.data.imap.from.length) {
    modules.abuse.data.imap.from.map(function(email) {
      search.push(['FROM', email]);
    });
  }
        ...
        f.on('message', function(msg) {
          ...
          parser.on('data', function(data) {
            if (data.type === 'text') {
              ...
              var re = new RegExp(config.domain + '\\/.*?\\/[a-z0-9_-]*', 'ig');
