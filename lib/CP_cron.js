...
if (hour === 1 || hour === 2 || hour === 3 || hour === 4) {
    ...
    function(year, key, callback) {
      ...
      CP_get.movies({ year: year }, -1, 'kinopoisk-vote-up', 1, true, function(
        ...
        if (movies && movies.length) {
          ...
          if (
            os.loadavg(1) > config.loadavg.one / 120 ||
            os.loadavg(5) > config.loadavg.five / 120 ||
            os.loadavg(15) > config.loadavg.fifteen / 120 ||
            Math.ceil(os.freemem()) < 100
          ) {
            callback('STOP');
          } else {
            setTimeout(function() {
  ...
  console.log('NOT START:', 'SITEMAP', 'hour', '1,2,3,4 !=', hour);
}
