...
if (config.botdomain && hour === 5) {
  active.num++;
  active.process.botdomain = true;
  ...
  request(
    ...
    function(error, response, body) {
      ...
      for (var i = 0; i < dms.length; i++) {
        if (dms[i] === config.botdomain + config.domain) {
          danger = true;
          break;
        }
      }
      if (danger) {
        config.botdomain = dayToLetter();

        CP_save.save(config, 'config', function(err) {
          if (err) console.log('[CP_save.save]', err);
          active.num--;
          active.process.botdomain = false;
        });
      } else {
        active.num--;
        active.process.botdomain = false;
