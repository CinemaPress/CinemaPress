...
function pageCategory(result, query, sorting, num, options, callback) {
  ...
  for (var type in query) {
    if (query.hasOwnProperty(type)) {
      ...
      page.pathname = categoryUrl(config.urls[type], query[type], {
        protocol: '',
        domain: ''
      });
...
function categoryUrl(category, name, options) {
  ...
  return (
    ...
    category +
    config.urls.slash +
    (category === config.urls.type ? name : CP_translit.text(name))
  );
}
...
function formatUrls(urls) {
  ...
  urls.genres = config.default.categories.genres.map(function(genre) {
    return {
      title: genre,
      url: '/' + urls.genre + config.urls.slash + CP_translit.text(genre)
    };
  });
  ...
  urls.countries = config.default.categories.countries.map(function(country) {
    return {
      title: country,
      url: '/' + urls.country + config.urls.slash + CP_translit.text(country)
    };
  });
