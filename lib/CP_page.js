...
function pageCategory(result, query, sorting, num, options, callback) {
  ...
  for (var type in query) {
    if (query.hasOwnProperty(type) && typeof query['page'] === 'undefined') {
      ...
      page.pathname = categoryUrl(config.urls[type], query[type], Object.assign({query: options.query || {}}, {
        protocol: '',
        domain: ''
      }));
...
function pageContent(result, url, num, sorting, options, callback) {
  ...
  if (
    ...
    (!/NoComment/i.test(content.tags) && !/NoRandom/i.test(content.tags))
  ) {
    page.codes.footer =
...
function pageContents(query, result, options, callback) {
  ...
  page.pathname = categoriesUrl(modules.content.data.url, {
    protocol: '',
    domain: ''
  });
  page.type = 'contents';
  ...
  if (options.query) {
    for (var q in options.query) {
      if (options.query.hasOwnProperty(q)) {
        options_query.push(q + '=' + CP_translit.text(options.query[q], undefined, q));
      }
    }
  }
...
function formatUrls(urls) {
  ...
  urls.genres = config.default.categories.genres.map(function(genre) {
    return {
      title: genre,
      url: '/' + urls.genre + config.urls.slash + CP_translit.text(genre, undefined, urls.genre)
    };
  });
  ...
  urls.countries = config.default.categories.countries.map(function(country) {
    return {
      title: country,
      url: '/' + urls.country + config.urls.slash + CP_translit.text(country, undefined, urls.country)
    };
  });
  ...
  for (var i = 1; i <= pages; i++) {
    ...
    if (number_prev >= 1) {
      pagination.prev.push({
        number: number_prev,
        link: (url.indexOf('?') + 1
          ? number_prev === 1 ? url : url
            .replace('?', '/' + number_prev + '?')
          : number_prev === 1 ? url : url + '/' + number_prev) + sorting_page
      });
    }
