...
module.exports = function() {
  return function(req, res, next) {
    ...
    var get_defense_domain = defense_domain.get(req.userinfo.ip);
    ...
    if (req.query && req.query.captcha) {
      ...
      if (get_defense_domain && get_defense_domain.captcha) {
        ...
        if (c1 === c2) {
          defense_domain.del(req.userinfo.ip);
      ...
      if (get_defense_agent && get_defense_agent.captcha) {
        ...
        if (c1 === c3) {
          defense_agent.del(req.userinfo.ip);
    ...
    if (get_defense_domain && config.defense.domain) {
      if (get_defense_domain.domains.indexOf(host_domain) === -1) {
        get_defense_domain.domains.push(host_domain);
        defense_domain.set(req.userinfo.ip, get_defense_domain);
      ...
      defense_domain.set(req.userinfo.ip, {
        domains: [host_domain]
      });
    }
