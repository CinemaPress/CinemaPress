  ...
  async.parallel(
    ...
    function(err, result) {
      ...
      if (
        ...
        script = script
          .replace('iframe-src', result['moonwalk'].src + autoplay)
          .replace(
            'iframe-translate',
            result['moonwalk'].translate.toUpperCase()
          )
          .replace('iframe-quality', result['moonwalk'].quality.toUpperCase());
      } else if (result['hdgo'].src) {
        script = script
          .replace('iframe-src', result['hdgo'].src + autoplay)
        ...
        if (json && !json.error && json.length) {
          ...
          for (var i = 0; i < json.length; i++) {
            if (season && episode && translate === json[i].translator_id) {
              iframe_url =
                getMoonlight(json[i].iframe_url) +
                '?season=' +
                season +
                '&episode=' +
                episode;
    ...
    function getMoonlight(iframe_url) {
      ...
      if (str && str[0]) {
        if (
          ...
          domain =
            domain[domain.length - 1] === '/' ? domain.slice(0, -1) : domain;
          domain =
            domain.indexOf('://') === -1 ? config.protocol + domain : domain;
